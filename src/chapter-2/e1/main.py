# Array exercises
# Custom array data structure that holds integers
...
class ReservedMemory:
    ...
    def __init__(self, size: int) -> None:
        ...
        if not 1 <= size <= 65536:
            raise (
                ValueError("Reserved memory size must be between 1 and 65536 bytes!")
            )
...
class IntArray:
    ...
    def __getitem__(self, k: int) -> int:
        ...
        stored_val = 0
        for byte_index in range(self._bytes_per_element):
            stored_val |= self._resmem[k * self._bytes_per_element + byte_index] << (
                8 * byte_index
            )
    ...
    def insert(self, index: int, val: int) -> None:
        ...
        # Copy the old reserved memory area up to the insertion point
        if self._resmem:
            new_resmem.copy(self._resmem, count=index * self._bytes_per_element)

        # Copy the remaining elements from the old reserved memory area
        if self._resmem:
            new_resmem.copy(
                self._resmem,
        ...
        # The new created reserved memory area will be the one to be used from now on
        self._resmem = new_resmem
